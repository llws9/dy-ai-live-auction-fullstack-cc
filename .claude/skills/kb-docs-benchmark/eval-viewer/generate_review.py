...
def build_run(root: Path, run_dir: Path) -> Optional[dict]:
    ...
    # QA benchmark writes prompt.md under outputs/ for each run.
    if not prompt:
        for candidate in [run_dir / "outputs" / "prompt.md", run_dir / "outputs" / "question.md"]:
            if candidate.exists():
                try:
                    prompt = candidate.read_text().strip()
                except OSError:
                    pass
                if prompt:
                    break
