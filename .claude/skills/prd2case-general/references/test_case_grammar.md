...
3. Special Regulations 
  a. First child of an `操作步骤` node should be `预期结果` and CANNOT be another `操作步骤`. If you want to describe a sequence of operations, group them in a single `操作步骤` node and separate them using numbered lines with line breaks.
...
**[priority]** P2
**[tag]** 标签1,标签2,标签3
**[img]**: https://bytest.bytedance.net/base/file/s/obj/iOSPackageBackUp/1773123589_ca5a_LgXKbNzcVofubNxz7MDc6zzrnid.png
####### **操作步骤** step1-1
######## **预期结果** assertion1
######## **操作步骤** step1-1-next
...
6. Other attributes of the node (each attribute should start on a new line)
...
7. Special node type: `测试内容`
  - This node is a special node that is ALWAYS attached to a leaf `功能点` node, and thus it SHOULD NOT start with `#`
  - A `测试内容` node is used in framework generation, and the content is free text.
  - example
```text
# 用例标题
## 功能
...
2. Fields that MUST be parsed (fields that occurred in Markdown format):
  - Why: these fields are the minimal contract to round-trip Markdown <-> JSON without losing structure/metadata.
  - If violated: conversion may drop tags/priority/links or break the tree, leading to wrong rendering or failed downstream processing.
