...
| `backend/product/go.mod` | Modify | 增 `require shared/llm` + `replace shared/llm => ../shared/llm`；增 `go-redis/v9`、`miniredis` |
...
| `backend/product/dao/category.go` | Modify | 追加 `Exists(ctx, id)` 适配 `CategoryExistsChecker` |
| `backend/product/main.go` | Modify | 装配 redis、shared/llm provider、copywriting handler；注册路由 |
...
```bash
git add backend/shared/llm/doubao_test.go
git commit -m "test(shared/llm): cover timeout/4xx/5xx/empty-choices"
```

---

## Task 5: `product-service` 引入依赖（shared/llm + go-redis + miniredis）
...
- [ ] **Step 2: 加 redis 与 miniredis**

```bash
cd backend/product && go get github.com/redis/go-redis/v9@v9.19.0 && go get github.com/alicebob/miniredis/v2@latest
```
...
Expected: 无错误；`go.mod` 中能看到 `require shared/llm v0.0.0` 与 `replace shared/llm => ../shared/llm`

- [ ] **Step 4: Commit**

```bash
git add backend/product/go.mod backend/product/go.sum
git commit -m "chore(product): add shared/llm replace, go-redis/v9 and miniredis deps"
```
...
type fakeCategoryExists struct{ ids map[int64]bool }

func (f *fakeCategoryExists) Exists(ctx context.Context, id int64) (bool, error) {
	return f.ids[id], nil
}
...
func TestCopywriting_Generate_Success(t *testing.T) {
	...
	svc := NewCopywritingService(fp, &fakeCategoryExists{ids: map[int64]bool{1: true}}, newRedis(t), "doubao-1.5-vision-pro")
...
func TestCopywriting_Generate_EmptyImages_400(t *testing.T) {
	fp := &fakeProvider{}
	svc := NewCopywritingService(fp, &fakeCategoryExists{}, newRedis(t), "m")
...
func TestCopywriting_Generate_CategoryNotExists_400(t *testing.T) {
	...
	svc := NewCopywritingService(fp, &fakeCategoryExists{ids: map[int64]bool{}}, newRedis(t), "m")
...
// CategoryExistsChecker 抽象类目存在性查询，便于测试。
...
type CopywritingService struct {
	...
	categoryChk  CategoryExistsChecker
	redis        *redis.Client
	defaultModel string
	now          func() time.Time
}

// NewCopywritingService 构造 service。
func NewCopywritingService(p sharedllm.Provider, c CategoryExistsChecker, r *redis.Client, defaultModel string) *CopywritingService {
...
func (s *CopywritingService) validate(ctx context.Context, req *CopywritingRequest) error {
	if len(req.Images) == 0 {
	...
	if req.CategoryID != nil {
		ok, err := s.categoryChk.Exists(ctx, *req.CategoryID)
		if err != nil {
...
func (s *CopywritingService) buildChatRequest(req *CopywritingRequest) *sharedllm.ChatRequest {
	...
	var info []string
	if req.CategoryID != nil {
		info = append(info, fmt.Sprintf("类目ID: %d", *req.CategoryID))
	}
...
git commit -m "feat(product/service): add CopywritingService with rate limit + JSON parse"
```
...
**Files:**
- Modify: `backend/product/dao/category.go` (新增 Exists 适配)
- Modify: `backend/product/main.go`

- [ ] **Step 1: 给 CategoryDAO 加 Exists 方法**

在 [category.go](file:///Users/bytedance/myself/coding/dy-ai-live-auction-fullstack-cc/backend/product/dao/category.go) 末尾追加：

```go
// Exists 检查类目是否存在（按 id）。
func (d *CategoryDAO) Exists(ctx context.Context, id int64) (bool, error) {
	var count int64
	err := d.db.WithContext(ctx).Model(&model.Category{}).Where("id = ?", id).Count(&count).Error
	return count > 0, err
}
```
...
```bash
git add backend/product/dao/category.go backend/product/main.go
git commit -m "feat(product): wire shared/llm provider, redis client and ai/copywriting route"
```
...
Plan 已落盘 `docs/superpowers/plans/2026-06-01-ai-copywriting-mvp-plan.md`，两种执行方式：

1. **Subagent-Driven（推荐）**：每个 Task 派一个 fresh subagent 写 + 跑测试，主线 review；适合 10 个独立 task 的场景。
2. **Inline Execution**：在当前 session 内顺序执行，每 2-3 个 task 一个 checkpoint。
