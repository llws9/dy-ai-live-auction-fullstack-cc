...
> 说明：I1 验证 engine 层 challenge→confirmed 流转，I2 验证 block 持久化。handler 层的状态码/risk_code 契约（I1b）在 Task 11 覆盖；前端重试（F1）在 Task 14 覆盖。本 task 不依赖端到端真实部署。
