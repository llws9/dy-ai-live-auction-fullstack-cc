...
If `HEAD` differs from `origin/main`, do not run `restart`. If `HEAD` matches `origin/main`, run `restart` and let the script precheck report ignored-local changes or block non-ignored local changes.
