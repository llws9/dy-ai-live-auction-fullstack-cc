...
func (o *Orchestrator) Run(ctx context.Context, p runner.ProgressEmitter) (*Report, error) {
	...
	seller := auction.Actor{UserID: o.cfg.SellerID, Username: fmt.Sprintf("merchant_%d", o.cfg.SellerID), Role: auction.RoleMerchant}
	...
	// ---------- setup ----------
	prodStep := o.cli.CreateProductAs(ctx, seller, auction.CreateProductReq{
	...
	ruleStep := o.cli.CreateAuctionRule(ctx, seller, rep.ProductID, auction.CreateAuctionRuleReq{
	...
	o.record(rep, p, 8, ruleStep)
	if !ruleStep.OK {
		return rep, fmt.Errorf("setup create_auction_rule failed: %s", ruleStep.Message)
	}
	...
	auctionStep := o.cli.CreateAuctionAs(ctx, seller, auction.CreateAuctionReq{
	...
	case len(orders) == 0:
		orderCheck.OK = true
		orderCheck.Message = "auction order is not generated by current service contract"
	default:
