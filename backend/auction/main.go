...
func main() {
	...
	internalAuctionCountHandler := handler.NewInternalAuctionCountHandler(auctionDAO)
...
func registerInternalRoutes(h *server.Hertz, internalAuth app.HandlerFunc, internalUserHandler *handler.InternalUserHandler, liveReminderHandler *handler.LiveReminderHandler, liveStreamStatsHandler *handler.LiveStreamStatsHandler, internalAuctionCountHandler *handler.InternalAuctionCountHandler) {
	internal := h.Group("/internal", internalAuth)
	if internalUserHandler != nil {
		internal.POST("/users/batch", internalUserHandler.BatchByIDs)
	}
	if internalAuctionCountHandler != nil {
		internal.POST("/auctions/count-by-live-streams", internalAuctionCountHandler.CountByLiveStreams)
	}
