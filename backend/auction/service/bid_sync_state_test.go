...
func TestBidService_PlaceBidSavesSyncState(t *testing.T) {
	db := setupServiceDB(t)
	require.NoError(t, db.AutoMigrate(&model.Auction{}, &model.Bid{}, &model.AuctionRule{}))

	mr := miniredis.RunT(t)
	rdb := redis.NewClient(&redis.Options{Addr: mr.Addr()})
	previousRedis := dao.RedisClient
	dao.RedisClient = rdb
	t.Cleanup(func() {
		dao.RedisClient = previousRedis
		_ = rdb.Close()
	})
	...
	require.NoError(t, dao.NewAuctionDAO(db).Create(ctx, auction))
	...
	svc := NewBidService(dao.NewAuctionDAO(db), dao.NewBidDAO(db), dao.NewAuctionRuleDAO(db), nil)
