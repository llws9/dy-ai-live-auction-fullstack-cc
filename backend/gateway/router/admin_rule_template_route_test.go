...
func TestApplyRuleTemplateRouteMerchantOnly(t *testing.T) {
	var calls atomic.Int64
	productMock := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
		require.Equal(t, "/api/v1/admin/products/101/apply-rule-template", r.URL.Path)
		require.Equal(t, "internal-secret", r.Header.Get("X-Internal-Token"))
		require.Equal(t, "merchant", r.Header.Get("X-User-Role"))
		w.WriteHeader(http.StatusOK)
	}))
	...
	w := ut.PerformRequest(h.Engine, http.MethodPost, "/api/v1/admin/products/101/apply-rule-template", nil, ut.Header{Key: "Authorization", Value: "Bearer " + merchantToken})
	require.Equal(t, http.StatusOK, w.Result().StatusCode())
	require.Equal(t, int64(1), calls.Load())

	calls.Store(0)
	w = ut.PerformRequest(h.Engine, http.MethodPost, "/api/v1/admin/products/101/apply-rule-template", nil, ut.Header{Key: "Authorization", Value: "Bearer " + adminToken})
	require.Equal(t, http.StatusForbidden, w.Result().StatusCode())
	require.Equal(t, int64(0), calls.Load())
}
