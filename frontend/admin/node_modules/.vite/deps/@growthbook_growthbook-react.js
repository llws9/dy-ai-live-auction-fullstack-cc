...
function hash(seed, value, version) {
  ...
  if (version === 1) {
    return hashFnv32a(value + seed) % 1e3 / 1e3;
  }
  return null;
}
...
function isURLTargeted(url, targets) {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    if (targets[i].include === false) {
      ...
      if (match) isIncluded2 = true;
    }
  }
  return isIncluded2 || !hasIncludeRules;
}
...
function getBucketRanges(numVariations, coverage, weights) {
  coverage = coverage === void 0 ? 1 : coverage;
  if (coverage < 0) {
    if (true) {
...
var base64ToBuf = (b) => Uint8Array.from(atob(b), (c) => c.charCodeAt(0));
...
var subscribedInstances = /* @__PURE__ */ new Map();
var cacheInitialized = false;
var cache = /* @__PURE__ */ new Map();
var activeFetches = /* @__PURE__ */ new Map();
var streams = /* @__PURE__ */ new Map();
var supportsSSE = /* @__PURE__ */ new Set();
function setPolyfills(overrides) {
  Object.assign(polyfills2, overrides);
}
...
function subscribe(instance) {
  ...
  const subs = subscribedInstances.get(key) || /* @__PURE__ */ new Set();
  subs.add(instance);
...
async function updatePersistentCache() {
  try {
    if (!polyfills2.localStorage) return;
...
async function fetchFeaturesWithCache({
  ...
  const existing = !cacheSettings.disableCache && !skipCache ? cache.get(cacheKey) : void 0;
...
function onNewFeatureData(key, cacheKey, data) {
  ...
  const existing = !cacheSettings.disableCache ? cache.get(cacheKey) : void 0;
...
function onSSEError(channel) {
  ...
  if (channel.errors > 3 || channel.src && channel.src.readyState === 2) {
    const delay = Math.pow(3, channel.errors - 3) * (1e3 + Math.random() * 1e3);
    disableChannel(channel);
    setTimeout(() => {
...
function createElementPropertyRecord(el, attr, getCurrentValue, setValue2, mutationRunner) {
  ...
  var record = {
    ...
    observer: new MutationObserver(function() {
      ...
      if (attr === "position" && currentValue2.parentNode === record.virtualValue.parentNode && currentValue2.insertBeforeNode === record.virtualValue.insertBeforeNode) return;
      if (currentValue2 === record.virtualValue) return;
...
function stopMutating(mutation, el) {
  ...
  if (index2 !== -1) record.mutations.splice(index2, 1);
  record.mutationRunner(record);
}
...
function attribute(selector, attribute2, mutate) {
  if (!validAttributeName.test(attribute2)) return nullController;
...
function getRegex(regex, insensitive = false) {
  ...
  if (!_regexCache[cacheKey]) {
    _regexCache[cacheKey] = new RegExp(regex.replace(/([^\\])\//g, "$1\\/"), insensitive ? "i" : void 0);
  }
...
function evalFeature(id, ctx) {
  ...
  if (feature.rules) {
    ...
    rules: for (const rule of feature.rules) {
      ...
      if (rule.bucketVersion !== void 0) exp.bucketVersion = rule.bucketVersion;
      if (rule.minBucketVersion !== void 0) exp.minBucketVersion = rule.minBucketVersion;
  ...
  return getFeatureResult(ctx, id, feature.defaultValue === void 0 ? null : feature.defaultValue, "defaultValue");
}
function runExperiment(experiment, featureId, ctx) {
  ...
  if (!foundStickyBucket) {
    const ranges = experiment.ranges || getBucketRanges(numVariations, experiment.coverage === void 0 ? 1 : experiment.coverage, experiment.weights);
  ...
  if (stickyBucketVersionIsBlocked) {
    ...
    return {
      result: getExperimentResult(ctx, experiment, -1, false, featureId, void 0, true)
    };
  }
  if (assigned < 0) {
  ...
  if ("force" in experiment) {
    ...
    return {
      result: getExperimentResult(ctx, experiment, experiment.force === void 0 ? -1 : experiment.force, false, featureId)
    };
  }
  ...
  const trackingCall = !trackingCalls.length ? void 0 : trackingCalls.length === 1 ? trackingCalls[0] : Promise.all(trackingCalls).then(() => {
  });
...
function isIncludedInRollout(ctx, seed, hashAttribute, fallbackAttribute, range, coverage, hashVersion) {
  if (!range && coverage === void 0) return true;
  ...
  if (n === null) return false;
  return range ? inRange(n, range) : coverage !== void 0 ? n <= coverage : true;
}
function getExperimentResult(ctx, experiment, variationIndex, hashUsed, featureId, bucket, stickyBucketUsed) {
  ...
  if (bucket !== void 0) res.bucket = bucket;
...
var GrowthBook = class {
  ...
  constructor(options) {
    ...
    this._completedChangeIds = /* @__PURE__ */ new Set();
    this._trackedFeatures = {};
    this.debug = !!options.debug;
    this._subscriptions = /* @__PURE__ */ new Set();
    this.ready = false;
    this._assigned = /* @__PURE__ */ new Map();
    this._activeAutoExperiments = /* @__PURE__ */ new Map();
    this._triggeredExpKeys = /* @__PURE__ */ new Set();
    this._initialized = false;
    this._redirectedUrl = "";
    this._deferredTrackingCalls = /* @__PURE__ */ new Map();
  ...
  _runAutoExperiment(experiment, forceRerun) {
    ...
    if (result.inExperiment) {
      ...
      if (changeType === "redirect" && result.value.urlRedirect && experiment.urlPatterns) {
        ...
        if (navigate) {
          if (isBrowser) {
            Promise.all([...trackingCall ? [promiseTimeout(trackingCall, this._options.maxNavigateDelay ?? 1e3)] : [], new Promise((resolve) => window.setTimeout(resolve, this._options.navigateDelay ?? delay))]).then(() => {
              try {
                navigate(url);
              } catch (e) {
                console.error(e);
...
// node_modules/@growthbook/growthbook/dist/esm/GrowthBookClient.mjs
var SDK_VERSION2 = loadSDKVersion();

// node_modules/@growthbook/growthbook/dist/esm/sticky-bucket-service.mjs
var StickyBucketService = class {
  constructor(opts) {
    opts = opts || {};
...
var StickyBucketServiceSync = class extends StickyBucketService {
...
var LocalStorageStickyBucketService = class extends StickyBucketService {
  constructor(opts) {
    opts = opts || {};
    super();
...
var ExpressCookieStickyBucketService = class extends StickyBucketServiceSync {
  /**
...
var BrowserCookieStickyBucketService = class extends StickyBucketServiceSync {
  /**
...
var RedisStickyBucketService = class extends StickyBucketService {
...
var GrowthBookContext = React.createContext({});
...
function FeaturesReady({
  ...
  return React.createElement(React.Fragment, null, ready || hitTimeout ? children : fallback || null);
}
function IfFeatureEnabled({
  children,
  feature
}) {
  return useFeature(feature).on ? React.createElement(React.Fragment, null, children) : null;
}
...
var withRunExperiment = (Component) => {
  const withRunExperimentWrapper = (props) => React.createElement(GrowthBookContext.Consumer, null, ({
    growthbook
  }) => {
...
var GrowthBookProvider = ({
  ...
  return React.createElement(GrowthBookContext.Provider, {
    value: {
      growthbook
    }
  }, children);
...
//# sourceMappingURL=@growthbook_growthbook-react.js.map
