...
const LiveRoomPage: React.FC = () => {
  ...
  const loadRanking = useCallback(async () => {
    ...
    try {
      const rankingResponse = await bidApi.getRanking(auctionId, 10);
    ...
    try {
      const bidsResponse = await auctionApi.getBids(auctionId);
      setRanking(normalizeRanking(extractList(bidsResponse)));
    } catch (error) {
      console.warn('获取出价记录失败:', error);
      setRanking([]);
    }
  }, [auctionId, normalizeRanking]);
  ...
  useEffect(() => {
    ...
    const loadLiveRoom = async () => {
      ...
      setLoading(true);
      try {
        const auctionData: Auction = await auctionApi.get(auctionId);
        if (cancelled) return;
