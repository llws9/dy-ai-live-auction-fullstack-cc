...
test.describe('竞拍流程 - 新版 H5 UI', () => {
  ...
  test('收藏标签展示新版空态', async ({ page }) => {
    ...
    await expect(page.getByText('暂无收藏竞拍')).toBeVisible();
    await expect(page.getByText('收藏接口待后端开放后接入。')).toBeVisible();
  });
  ...
  test('未登录出价跳转登录并携带 redirect', async ({ page }) => {
    ...
    await page.getByPlaceholder('输入出价金额').fill('12900');
  ...
  test('出价金额过低展示新版 toast', async ({ page }) => {
    ...
    await page.getByPlaceholder('输入出价金额').fill('12800');
    await page.getByRole('button', { name: '出价' }).click();
  ...
  test('快捷加价按钮会填充出价输入框', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('输入出价金额')).toHaveValue('12900');
  });
